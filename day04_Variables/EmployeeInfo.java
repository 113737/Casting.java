package day04_Variables;

public class EmployeeInfo {

    public static void main(String[] args) {
        String employeeName = "Kubra";
        int age = 36;
        char gender = 'F';
        String company = "Education";
        String jobTitle = "Teacher";
        double yearsOfExperience = 11;
        int salary = 15000;
        boolean isFullTime = true;
        boolean isMarried = true;
        String employedId = "123";
        String SSN = "(123)4567899";

        System.out.println("employeeName = " + employeeName);
        System.out.println("age = " + age);
        System.out.println("gender = " + gender);
        System.out.println("company = " + company);
        System.out.println("jobTitle = " + jobTitle);
        System.out.println("yearsOfExperience = " + yearsOfExperience);
        System.out.println("salary = " + salary);
        System.out.println("isFullTime = " + isFullTime);
        System.out.println("isMerried = " + isMarried);
        System.out.println("employedId = " + employedId);
        System.out.println("SSN = " + SSN);

    }
}
/*
6. Create a class named EmployeeInfo. declare the following variables:
				1. name (String)
				2. age (int)
				3. gender (char)
				4. company (String)
				5. jobTitle (String)
				6. yearsOfExpereince (double)
				7. salary (int)
				8. isFullTime (boolean)
				9. isMarried (boolean)
				10. employeeId (String)
				11. SSN (String)

 */